from __future__ import annotations

import ast
import hashlib
import json
import unittest
from pathlib import Path


STEP_ROOT = Path(__file__).resolve().parents[1]
REPO_ROOT = STEP_ROOT.parents[1]
CONTRACT_PATH = (
    STEP_ROOT
    / "phases"
    / "phase_05_source_robustness"
    / "experiment_contract_v3.json"
)
RUNNER_PATH = STEP_ROOT / "scripts" / "run_phase5_v3_consensus.py"
EXPECTED_CONTRACT_SHA256 = (
    "828cc5aa45724518e10808929589722133ce336ee272cb8bda20b5089c0dc980"
)
EXPECTED_OUTPUTS = {
    "metrics.json",
    "base_grid.csv",
    "fusion_grid.csv",
    "inner_search.csv",
    "outer_fold_metrics.csv",
    "nested_oof_predictions.csv",
    "full_development_selection.csv",
    "selected_oof_predictions.csv",
    "per_class_metrics.csv",
    "diagnostic_slices.csv",
    "confusion_pairs.csv",
    "confusion_matrix.csv",
    "confusion_matrix.png",
}


class Phase5V3FrozenContractTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.raw = CONTRACT_PATH.read_bytes()
        cls.contract = json.loads(cls.raw)

    def test_contract_is_frozen_development_only_and_exact(self) -> None:
        self.assertEqual(hashlib.sha256(self.raw).hexdigest(), EXPECTED_CONTRACT_SHA256)
        self.assertEqual(self.contract["iteration"], "v3")
        self.assertEqual(self.contract["status"], "frozen_before_fit")
        self.assertEqual(self.contract["scope"]["sealed_test_access"], "forbidden")
        self.assertEqual(self.contract["scope"]["development_image_count"], 1693)

    def test_grid_cache_and_transition_contracts_are_bounded(self) -> None:
        heads = self.contract["consensus_heads"]
        fusion = self.contract["consensus_fusion"]
        count = 1 + len(heads["families"]) * (
            len(fusion["versions"]) - 1
        ) * len(fusion["blend_weights"])
        self.assertEqual(count, 28)
        self.assertEqual(fusion["grid_configuration_count_including_none"], 28)
        self.assertEqual(len(self.contract["cache_allowlist"]), 7)
        self.assertEqual(
            sum(len(value["arrays"]) for value in self.contract["cache_allowlist"].values()),
            9,
        )
        self.assertIn("every promotion result passes", self.contract["promotion_gates"]["phase_6_transition"])

    def test_v1_gates_non_regression_and_stability_are_required(self) -> None:
        v1_contract = json.loads(
            (CONTRACT_PATH.parent / "experiment_contract.json").read_text()
        )
        for key, value in v1_contract["promotion_gates"].items():
            self.assertEqual(self.contract["promotion_gates"][key], value)
        self.assertTrue(self.contract["promotion_gates"]["fusion_recipe_stability_required"])
        self.assertEqual(
            set(self.contract["promotion_gates"]["v1_non_regression"]),
            {"accuracy_min", "macro_f1_min", "source_robustness_min"},
        )

    def test_required_outputs_are_exact(self) -> None:
        self.assertEqual(set(self.contract["output_contract"]["required"]), EXPECTED_OUTPUTS)


class Phase5V3RunnerStaticTests(unittest.TestCase):
    @classmethod
    def setUpClass(cls) -> None:
        cls.source = RUNNER_PATH.read_text(encoding="utf-8")
        cls.tree = ast.parse(cls.source)
        cls.functions = {
            node.name for node in ast.walk(cls.tree) if isinstance(node, ast.FunctionDef)
        }

    def test_runner_pins_contract_and_uses_only_v3_loader(self) -> None:
        self.assertIn(EXPECTED_CONTRACT_SHA256, self.source)
        self.assertIn("load_allowlisted_embeddings_v3", self.source)
        self.assertNotIn("np.load", self.source)
        self.assertNotIn("test.csv", self.source.lower())
        attributes = {
            node.func.attr
            for node in ast.walk(self.tree)
            if isinstance(node, ast.Call) and isinstance(node.func, ast.Attribute)
        }
        self.assertFalse(attributes & {"glob", "rglob", "iglob"})

    def test_runner_contains_nested_sequential_lifecycle(self) -> None:
        required = {
            "fit_auxiliary_oof",
            "evaluate_fusion_oof_grid",
            "select_fusion_candidates",
            "fit_outer_procedure",
            "stability",
            "evaluate_promotion_gates",
            "main",
        }
        self.assertFalse(required - self.functions)
        main = next(
            node for node in self.tree.body if isinstance(node, ast.FunctionDef) and node.name == "main"
        )
        source = ast.get_source_segment(self.source, main) or ""
        self.assertLess(source.index("base_selection ="), source.index("fusion_results ="))
        self.assertIn("uncorrected", source)
        self.assertIn("full_fusion_selection", source)

    def test_fit_boundaries_are_thread_limited_and_source_disjoint(self) -> None:
        self.assertIn("_assert_source_disjoint_partitions", self.source)
        self.assertGreaterEqual(self.source.count("threadpool_limits(limits=1)"), 2)
        self.assertIn('"blas_threads_per_fit": 1', self.source)

    def test_outputs_disclaim_calibration_and_sealed_evaluation(self) -> None:
        self.assertIn('"sealed_test_evaluated": False', self.source)
        self.assertIn('"calibrated": False', self.source)
        self.assertIn('"threshold_selection_used": False', self.source)
        strings = {
            node.value
            for node in ast.walk(self.tree)
            if isinstance(node, ast.Constant) and isinstance(node.value, str)
        }
        self.assertFalse(EXPECTED_OUTPUTS - strings)


if __name__ == "__main__":
    unittest.main()
